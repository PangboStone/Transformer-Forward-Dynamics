...
# --- 1. 实验配置 ---
# 实验名称 (所有输出文件都将以此为前缀)
EXPERIMENT_NAME = f'transformer_large_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
...
def plot_comparison(ground_truth, sbs_predictions, full_predictions, save_path):
    num_plots = 7
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(os.path.join(TENSORBOARD_DIR, EXPERIMENT_NAME))
    ...
    train_trajectories = all_trajectories[:VALIDATION_TRAJECTORY_INDEX] + all_trajectories[
                                                                          VALIDATION_TRAJECTORY_INDEX + 1:]
    ...
    # --- 模型训练 ---
    log_print("\n[2/5] 正在初始化和训练模型...")
    model = TransformerModel(INPUT_SIZE, D_MODEL, N_HEAD, NUM_ENCODER_LAYERS, DIM_FEEDFORWARD, OUTPUT_SIZE, DROPOUT).to(
        DEVICE)
    loss_function = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)

    best_val_loss = float('inf')
    model_path = os.path.join(MODEL_SAVE_DIR, f'{EXPERIMENT_NAME}_best_model.pth')
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            log_print(f"  -> Better model (Epoch {epoch+1}) saved to: {model_path}")
    ...
    # --- 结果可视化 ---
    log_print("\n[5/5] 正在生成可视化图表...")
    plot_path = os.path.join(PLOT_SAVE_DIR, f'{EXPERIMENT_NAME}_prediction_plot.png')
    plot_comparison(
